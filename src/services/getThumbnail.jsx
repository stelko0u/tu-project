// export default function getThumbnailUrl(originalUrl) {
...
//   const decodedPath = decodeURIComponent(encodedPath.split("?")[0]); // cars/pic1.png

//   const parts = decodedPath.split("/"); // ['cars', 'pic1.png']
//   if (parts.length < 2) return originalUrl;

//   const folder = parts[0]; // 'cars'
//   const originalFile = parts[1]; // 'pic1.png'

//   // Изваждам името без разширение
//   const baseName = originalFile.replace(/\.[^/.]+$/, ""); // pic1

//   // Сглобявам името на thumbnail-a
//   const thumbnailFileName = `${baseName}_200x200.webp`; // pic1_200x200.webp

//   const thumbnailPath = `${folder}/thumbnails/${thumbnailFileName}`;

//   const bucket = "car-project-5ba3d.firebasestorage.app"; // ✅ поправен bucket
//   const encodedThumbnail = encodeURIComponent(thumbnailPath);

//   return `https://firebasestorage.googleapis.com/v0/b/${bucket}/o/${encodedThumbnail}?alt=media`;
// }
...
export default async function getThumbnailUrl(originalUrl) {
  ...
  // Проверка дали thumbnail съществува
  try {
